...
router.post('/v6/l2/users/add/org-check', function (req, res) {
    const chosenOrgName = req.session.data['organisation-name-existing'].toLowerCase();
    const orgName = req.session.data['organisation-name'];
    ...
    if (
        ...
        res.redirect('/v6/l2/users/add/details');
    } else {
        req.session.data['org-error'] = true;

        res.redirect('/v6/l2/users/add/start');
    }
});

router.post('/v6/l2/orgs/add/address-branch', function (req, res) {
    const checkChoice = req.session.data['add-org-address'];

    if (checkChoice === 'yes') {
        res.redirect('/v6/l2/orgs/add/address');
    } else {
        res.redirect('/v6/l2/orgs/add/contacts-check');
    }
});

router.post('/v6/l2/orgs/add/contacts-branch', function (req, res) {
    const checkChoice = req.session.data['add-org-contacts'];

    if (checkChoice === 'yes') {
        res.redirect('/v6/l2/orgs/add/contact');
    } else {
        res.redirect('/v6/l2/orgs/add/summary');
    }
});

router.post('/v6/l2/orgs/add/contacts-2-branch', function (req, res) {
    const checkChoice = req.session.data['add-additional-org-contact'];

    if (checkChoice === 'yes') {
        res.redirect('/v6/l2/orgs/add/contact-2');
    } else {
        res.redirect('/v6/l2/orgs/add/summary');
    }
});

router.post('/v6/l2/orgs/add/contacts-3-branch', function (req, res) {
    const checkChoice = req.session.data['add-another-additional-org-contact'];

    if (checkChoice === 'yes') {
        res.redirect('/v6/l2/orgs/add/contact-3');
    } else {
        res.redirect('/v6/l2/orgs/add/summary');
    }
});

router.post('/v6/l2/orgs/add/details-branch', function (req, res) {
    const orgCompleteDetails = req.session.data['org-complete-details'].toLowerCase();
    const orgAddress = req.session.data['address-line-1'];
    const orgContact1 = req.session.data['contact-1-first-name'];
    const orgContact2 = req.session.data['contact-2-first-name'];
    ...
    if (orgCompleteDetails === 'yes') {
        if (orgContact1 && !orgContact2) {
            ...
            res.redirect('/v6/l2/orgs/add/address-only');
        }
    } else if (orgCompleteDetails === 'no') {
        res.redirect('/v6/l2/orgs/view/custom');
    }
});

router.post('/v6/l2/orgs/view/actions-branch', function (req, res) {
    const orgAction = req.session.data['org-task'];

    console.log(orgAction);

    if (orgAction == 'delete') {
        res.redirect('/v6/l2/orgs/edit/delete-check');
    } else if (orgAction == 'delete-users-by-team') {
        res.redirect('/v6/l2/orgs/edit/delete-by-team-check');
    } else if (orgAction == 'delete-users-by-service') {
        res.redirect('/v6/l2/orgs/edit/delete-by-service-check');
    }
});

router.post('/v6/l2/orgs/edit/delete', function (req, res) {
    if (req.session.data['delete-check'] === 'yes') {
        res.redirect('/v6/l2/orgs/edit/delete-confirm');
    } else {
        res.redirect('/v6/l2/orgs/view/all');
    }
});

router.post('/v6/l2/orgs/edit/suspend', function (req, res) {
    if (req.session.data['suspend-check'] === 'yes') {
        res.redirect('/v6/l2/orgs/edit/suspend-confirm');
    } else {
        res.redirect('/v6/l2/orgs/view/all');
    }
});

router.post('/v6/l2/users/view/edit-branch', function (req, res) {
    const editChoice = req.session.data['user-task'];

    if (editChoice === 'reinvite') {
        res.redirect('/v6/l2/users/edit/reinvite');
    } else if (editChoice === 'delete') {
        res.redirect('/v6/l2/users/edit/delete-check');
    }
});

router.post('/v6/l2/users/edit/delete-branch', function (req, res) {
    const deleteChoice = req.session.data['delete-check'];

    if (deleteChoice === 'yes') {
        res.redirect('/v6/l2/users/edit/delete-confirm');
    } else {
        res.redirect('/v6/l2/users/view/single');
    }
});

router.post('/v6/l2/users/register/app-branch', function (req, res) {
    if (req.session.data['app-backup'] === 'yes') {
        res.redirect('/v6/l2/users/register/qr');
    } else {
...
/** All level 3 (Org admin users) routes */
router.post('/v6/l3/users/register/app-branch', function (req, res) {
    if (req.session.data['app-backup'] === 'yes') {
        res.redirect('/v6/l3/users/register/qr');
    } else {
        res.redirect('/v6/l3/users/register/complete');
    }
});

router.post('/v6/l3/users/view/edit-branch', function (req, res) {
    const editChoice = req.session.data['user-task'];

    if (editChoice === 'reinvite') {
        res.redirect('/v6/l3/users/edit/reinvite');
    } else if (editChoice === 'delete') {
        res.redirect('/v6/l3/users/edit/delete-check');
    }
});

router.post('/v6/l3/users/edit/delete-branch', function (req, res) {
    const deleteChoice = req.session.data['delete-check'];

    if (deleteChoice === 'yes') {
        res.redirect('/v6/l3/users/edit/delete-confirm');
    } else {
        res.redirect('/v6/l3/users/view/single');
    }
});

/** All level 4 (Standard users) routes */
router.post('/v6/l4/users/register/app-branch', function (req, res) {
    if (req.session.data['app-backup'] === 'yes') {
        res.redirect('/v6/l4/users/register/qr');
    } else {
...
/** Onboarding, is service contact also an admin */
router.post('/v6/add-service/admin-branch', function (req, res) {
    const numContacts = req.session.data['contacts-total'];
    ...
    if (thereAreAdmins) {
        ...
        res.redirect('/v6/add-service/support');
    } else {
        console.log('Go to add service admins');

        res.redirect('/v6/add-service/admins');
    }
});
 ...
 * Routes picked up from 'config' on the index view
 */
router.post('/v6/index-check', function (req, res) {
    const routeChosen = req.session.data['prototype-route'];

    switch (routeChosen) {
        case 'l2-register':
            res.redirect('/v6/l2/users/register/email');
            break;
        case 'l3-register':
            res.redirect('/v6/l3/users/register/email');
            break;
        case 'l4-register':
            res.redirect('/v6/l4/users/register/email');
            break;
        case 'l2-reset':
        ...
        case 'l2-dashboard':
            ...
            break;
        case 'l3-dashboard':
            res.redirect('/v6/l3/email');
            break;
        case 'l4-dashboard':
            res.redirect('/v6/l4/email');
            break;
        default:
            res.redirect('/v6/l2/email');
            break;
...
router.post('/v7/l2/users/add/org-check', function (req, res) {
    const chosenOrgName = req.session.data['organisation-name-existing'].toLowerCase();
    const orgName = req.session.data['organisation-name'];
    ...
    if (
        ...
        res.redirect('/v7/l2/users/add/details');
    } else {
        req.session.data['org-error'] = true;
...
router.post('/v7/l2/orgs/add/address-branch', function (req, res) {
    ...
    if (checkChoice === 'yes') {
        res.redirect('/v7/l2/orgs/add/address');
    } else {
        res.redirect('/v7/l2/orgs/add/contacts-check');
    }
});

router.post('/v7/l2/orgs/add/contacts-branch', function (req, res) {
    const checkChoice = req.session.data['add-org-contacts'];

    if (checkChoice === 'yes') {
        res.redirect('/v7/l2/orgs/add/contact');
    } else {
        res.redirect('/v7/l2/orgs/add/summary');
    }
});

router.post('/v7/l2/orgs/add/contacts-2-branch', function (req, res) {
    const checkChoice = req.session.data['add-additional-org-contact'];

    if (checkChoice === 'yes') {
        res.redirect('/v7/l2/orgs/add/contact-2');
    } else {
        res.redirect('/v7/l2/orgs/add/summary');
    }
});

router.post('/v7/l2/orgs/add/contacts-3-branch', function (req, res) {
    const checkChoice = req.session.data['add-another-additional-org-contact'];

    if (checkChoice === 'yes') {
        res.redirect('/v7/l2/orgs/add/contact-3');
    } else {
        res.redirect('/v7/l2/orgs/add/summary');
    }
});

router.post('/v7/l2/orgs/add/details-branch', function (req, res) {
    const orgCompleteDetails = req.session.data['org-complete-details'].toLowerCase();
    const orgAddress = req.session.data['address-line-1'];
    const orgContact1 = req.session.data['contact-1-first-name'];
    const orgContact2 = req.session.data['contact-2-first-name'];
    ...
    if (orgCompleteDetails === 'yes') {
        if (orgContact1 && !orgContact2) {
            ...
            res.redirect('/v7/l2/orgs/add/address-only');
        }
    } else if (orgCompleteDetails === 'no') {
        res.redirect('/v7/l2/orgs/view/custom');
    }
});

router.post('/v7/l2/orgs/view/actions-branch', function (req, res) {
    const orgAction = req.session.data['org-task'];

    console.log(orgAction);

    if (orgAction == 'delete') {
        res.redirect('/v7/l2/orgs/edit/delete-check');
    } else if (orgAction == 'delete-users-by-team') {
        res.redirect('/v7/l2/orgs/edit/delete-by-team-check');
    } else if (orgAction == 'delete-users-by-service') {
        res.redirect('/v7/l2/orgs/edit/delete-by-service-check');
    }
});

router.post('/v7/l2/orgs/edit/delete', function (req, res) {
    if (req.session.data['delete-check'] === 'yes') {
        res.redirect('/v7/l2/orgs/edit/delete-confirm');
    } else {
        res.redirect('/v7/l2/orgs/view/all');
    }
});

router.post('/v7/l2/orgs/edit/suspend', function (req, res) {
    if (req.session.data['suspend-check'] === 'yes') {
        res.redirect('/v7/l2/orgs/edit/suspend-confirm');
    } else {
        res.redirect('/v7/l2/orgs/view/all');
    }
});

router.post('/v7/l2/users/view/edit-branch', function (req, res) {
    const editChoice = req.session.data['user-task'];

    if (editChoice === 'reinvite') {
        res.redirect('/v7/l2/users/edit/reinvite');
    } else if (editChoice === 'delete') {
        res.redirect('/v7/l2/users/edit/delete-check');
    }
});

router.post('/v7/l2/users/edit/delete-branch', function (req, res) {
    const deleteChoice = req.session.data['delete-check'];

    if (deleteChoice === 'yes') {
        res.redirect('/v7/l2/users/edit/delete-confirm');
    } else {
        res.redirect('/v7/l2/users/view/single');
    }
});

router.post('/v7/l2/users/register/app-branch', function (req, res) {
    if (req.session.data['app-backup'] === 'yes') {
        res.redirect('/v7/l2/users/register/qr');
    } else {
...
/** All level 3 (Org admin users) routes */
router.post('/v7/l3/users/register/app-branch', function (req, res) {
    if (req.session.data['app-backup'] === 'yes') {
        res.redirect('/v7/l3/users/register/qr');
    } else {
        res.redirect('/v7/l3/users/register/complete');
    }
});

router.post('/v7/l3/users/view/edit-branch', function (req, res) {
    const editChoice = req.session.data['user-task'];

    if (editChoice === 'reinvite') {
        res.redirect('/v7/l3/users/edit/reinvite');
    } else if (editChoice === 'delete') {
        res.redirect('/v7/l3/users/edit/delete-check');
    }
});

router.post('/v7/l3/users/edit/delete-branch', function (req, res) {
    const deleteChoice = req.session.data['delete-check'];

    if (deleteChoice === 'yes') {
        res.redirect('/v7/l3/users/edit/delete-confirm');
    } else {
        res.redirect('/v7/l3/users/view/single');
    }
});

/** All level 4 (Standard users) routes */
router.post('/v7/l4/users/register/app-branch', function (req, res) {
    if (req.session.data['app-backup'] === 'yes') {
        res.redirect('/v7/l4/users/register/qr');
    } else {
...
/** Onboarding, is service contact also an admin */
router.post('/v7/add-service/admin-branch', function (req, res) {
    const numContacts = req.session.data['contacts-total'];
    ...
    if (thereAreAdmins) {
        ...
        res.redirect('/v7/add-service/support');
    } else {
        console.log('Go to add service admins');

        res.redirect('/v7/add-service/admins');
    }
});
...
/** Onboarding, add custom data */
router.post('/v7/add-service/custom-data-branch', function (req, res) {
    console.log(req.session.data['prototype-service']);

    if (req.session.data['custom-data-check'] === 'yes') {
        res.redirect('/v7/add-service/group-name');
    } else {
        res.redirect('/v7/add-service/summary');
    }
});
 ...
 * Routes picked up from 'config' on the index view
 */
router.post('/v7/index-check', function (req, res) {
    const routeChosen = req.session.data['prototype-route'];

    switch (routeChosen) {
        case 'l2-register':
            res.redirect('/v7/l2/users/register/email');
            break;
        case 'l3-register':
            res.redirect('/v7/l3/users/register/email');
            break;
        case 'l4-register':
            res.redirect('/v7/l4/users/register/email');
            break;
        case 'l2-reset':
            res.redirect('/v7/l2/users/reset-password/start');
            break;
        case 'l4-reset':
            res.redirect('/v7/l4/users/reset-password/start');
            break;
        case 'l2-dashboard':
            res.redirect('/v7/l2/email');
            break;
        case 'l3-dashboard':
            res.redirect('/v7/l3/email');
            break;
        case 'l4-dashboard':
            res.redirect('/v7/l4/email');
            break;
        default:
            res.redirect('/v7/l2/email');
            break;
